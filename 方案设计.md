...
### 方案 A: Railway (推荐，操作简单)
1.  注册 [Railway](https://railway.app/) 账号 (支持 GitHub 登录)。
2.  点击 **New Project** -> **Deploy from GitHub repo** (需要先将代码上传到 GitHub)。
3.  Railway 会自动识别 `requirements.txt` 并安装依赖。
4.  **关键配置**:
    *   在 Railway 项目设置 (Variables) 中，添加变量 `BOT_TOKEN`，值为您的 Token。
    *   (或者直接在代码 `config.py` 中写死 Token)。
5.  **FFmpeg 支持**:
    *   Railway 可能需要配置 Nixpacks 才能支持 FFmpeg。在项目根目录创建 `nixpacks.toml` 文件，内容如下：
        ```toml
        [phases.setup]
